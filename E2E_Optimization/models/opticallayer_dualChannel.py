...
def gen_OOFphase(Phi_list, N_B):
    ...
    N = N_B
    x0 = np.linspace(-1.55, 1.55, N) # 25/23 =1.09, 45/38 = 1.26, 45/29 = 1.55
    #xx, yy = torch.tensor(np.meshgrid(x0, x0)).to(device="cuda:0")
    meshgrid = np.meshgrid(x0, x0)
    meshgrid_array = np.array(meshgrid)
    xx, yy = torch.tensor(meshgrid_array).to(device="cuda:0")
    OOFphase = torch.tensor(np.empty([len(Phi_list), N, N, 2], dtype=np.float32)).to(device="cuda:0")
    for j in range(len(Phi_list)):
        Phi = Phi_list[j]
        OOFphase[j, :, :, 0] = Phi * (xx ** 2 + yy ** 2)
        OOFphase[j, :, :, 1] = Phi * (xx ** 2 + yy ** 2) * 460/600 ## correct based on wavelengths
    return OOFphase
...
def gen_PSFs(h, OOFphase, wvls, idx, N_B):
    ...
    phase_B1 = 2 * np.pi / wvls[0] * (n - 1) * h + OOFphase_B
    phase_B2 = 2 * np.pi / wvls[1] * (n - 1) * h + OOFphase_B
    
    complex_idx = torch.complex(idx,torch.zeros_like(idx))
    complex_exp1 = torch.complex(torch.zeros_like(phase_B1),phase_B1)
    complex_exp2 = torch.complex(torch.zeros_like(phase_B2),phase_B2)
    Pupil_B1 = complex_idx * torch.exp(complex_exp1)
    Pupil_B2 = complex_idx * torch.exp(complex_exp2)
 
    Norm_B =  torch.tensor(float(N_B * N_B * torch.sum(idx ** 2)), dtype=torch.float32)
    PSF_B1 = torch.divide(torch.square(torch.abs(torch.fft.fftshift(torch.fft.fft2(Pupil_B1, dim=(-2, -1))))), Norm_B)
    PSF_B1 = torch.unsqueeze(PSF_B1, -1)
    PSF_B2 = torch.divide(torch.square(torch.abs(torch.fft.fftshift(torch.fft.fft2(Pupil_B2, dim=(-2, -1))))), Norm_B)
    PSF_B2 = torch.unsqueeze(PSF_B2, -1)
    
  #  print(PSF_B1.shape)
    PSF_B = torch.cat([PSF_B1, PSF_B2], dim=3)
  #  print(PSF_B.shape)

    return PSF_B #torch.unsqueeze(PSF_B, -1)
...
def blurImage_diffPatch_diffDepth(RGB, PSFs):
  ...
  #  print(RGB.shape)
    ...
    blur = torch.cat([blur_channel1, blur_channel2], dim=0)
  #  print(blur_channel1.shape)
  #  print(blur.shape)
    
    return blur.permute(1, 0, 2, 3)
...
class fft_conv (nn.Module):
    ...
    def __init__(self, gamma, a_zernike_init, Phi_list, N_B, wvls, a_zernike_fix, idx, u2):
        ...
        a_zernike_learn.data = torch.clamp(a_zernike_learn.data, torch.tensor(-wvls[0] / 2).to(device="cuda:0"), torch.tensor(wvls[0] / 2).to(device="cuda:0"))
